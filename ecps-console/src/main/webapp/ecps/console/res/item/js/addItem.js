$(document)
		.ready(
				function() {
					...
					fck.BasePath = path
							+ "/ecps/console/res/plugins/fckeditor/";
					// 上传图片的地址
					fck.Config["ImageUploadURL"] = path
							+ "/upload/uploadForFck.do";
					fck.Height = 400;
					fck.ReplaceTextarea();
...
$(function() {
	...
	$("#tabs a").each(function() {
		...
		$(this).click(function() {
			...
			if (ref == '#tab_2') {
				...
				// FCKeditorAPI.GetInstance('itemDesc').EditorDocument.body.innerHTML
				// = '12123123';
			}
		});
	});
	...
	$("#button2").click(
			function() {
				...
				var b = a.replace(/clickRemove\('#sp\_\d+'\)/g,
						"clickRemove('#sp_" + d + "')");

				$("#button2").parent().parent().before(
						"<div class='sp_0' id='sp_" + d + "'>" + b + "</div>");
				$("#divNum").val(d);
			});
