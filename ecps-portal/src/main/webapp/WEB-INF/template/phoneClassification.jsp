...
<body>
	...
	<div class="bg_gray mt ofc">
			...
			<ul class="uls i_150x150 x4_150x150b">
				...
				<li>
					<a href="${path}/html/${item.itemId}.html" title="${item.itemName}" target="_blank" class="pic"><img src="${request_file_path}${item.imgs}" alt="${item.itemName}" /></a>
					<dl>
						<!-- dt 10个文字+... -->
						<dt><a href="${path}/html/${item.itemId}.html" title="${item.itemName}" target="_blank">${item.itemName}</a></dt>
						<dd class="h40">${item.itemName}</dd>
						<dd class="orange">￥${item.promotion}</dd>
						<dd><a href="${path}/html/${item.itemId}.html" title="购买裸机" class="inb btn70x21 mr">购买裸机</a><a href="./productDetail.jsp" title="优惠购机" class="inb btn70x21">优惠购机</a></dd>
					</dl>
					<img src="$" alt="热门" class="type" />
				</li>
				</c:forEach>
			</ul>
