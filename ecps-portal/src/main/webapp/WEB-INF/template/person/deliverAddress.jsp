...
<script src="${path}/res/js/person/deliverAddress.js"></script>
	...
	<div class="r wr profile">
		...
		<div class="confirm">
			...
			<div class="ofc">
				...
				<td>${addr.zipCode}</td>
				<td>${addr.phone}</td>
				<c:if test="${addr.defaultAddr==1}"><td >默认地址</td></c:if>
				<c:if test="${addr.defaultAddr==0}"><td class="def"><a href="javascript:void(0);" title="设为默认">设为默认</a></td></c:if>
				...
				<form id="jvForm" action="${path }/user/login/saveAndUpdateAddr.do" method="post">
					<input type="hidden" name="update" id="update" value=""/>
					<ul class="uls form">
					<li id="errorName" class="errorTip" style="display:none">${error}</li>
					<li>
						<label for="username"><samp>*</samp>收货人姓名：</label>
						<span class="bg_text"><input type="text" id="username" name="shipName" vld="{required:true}" maxLength="100" /></span>
						<span class="pos"><span class="tip okTip">&nbsp;</span></span>
					</li>
					<li>
						<label for="residence"><samp>*</samp>地　　址：</label>
						<span class="word">
						<select name="province" onchange="getChild(this,'province')">
							<option value="-1" selected >省/直辖市</option>
							<c:forEach items="${areas}" var="area">
								<option value="${area.areaName}"  id="${area.areaId}">${area.areaName}</option>
							</c:forEach>
							
						</select><select name="city" id="areacity" onchange="getChild(this,'city')">
					...
					<li>
						...
						<span class="bg_text"><input type="text" id="nick" name="addr" maxLength="32" vld="{required:true}" /></span>
					...
					<li>
						...
						<span><input type="checkbox" checked="checked" id="defaultAddr" name="defaultAddr" value="1" />设为默认收货地址</span>
					</li>
