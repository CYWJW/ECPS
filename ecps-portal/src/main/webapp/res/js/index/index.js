$(function() {
	...
	$('.filter li a').mousedown(function() {
		...
		$('.filter li a').each(function() {
			...
			if (here == "here") {
				// 获取类型
				if ($(this).attr("fType") == "price") {
					price = $(this).attr("fValue");
				}
				else if ($(this).attr("fType") == "brandId") {
					brandId = $(this).attr("fValue");
				}
				else if ($(this).attr("fType") == "featValues" && $(this).attr("fValue") != "") {
					featValues=featValues+$(this).attr("fValue")+",";
				}
		...
		//alert("price===>"+price+"  brandName===>"+brandId+"  featureName===>"+featValues+"  keyWords===>"+keyWords);
		$("#itemListIframe").attr("src",path+"/item/phoneClassification.do?price="+price+"&brandId="+brandId+"&featValues="+featValues+"&keyWords="+keyWords)
	});
	...
	/*
	 * sales sales_down sales_up price price_down price_up time time_down
	 * time_up
	 */
